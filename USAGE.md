...
**Counties** (US only) — read from `us_counties.geojson`. Uses the `CNTY` field (e.g. `WA,King`). County/Parish/Borough suffixes are stripped automatically before matching.
